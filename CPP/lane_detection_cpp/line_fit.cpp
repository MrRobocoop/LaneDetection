...
bool line_fit(mat input_img, vector<float> &ret, int windows_size, int line_margin, int vertical_margin, cv::Mat &temp){
    ...
    vec center_line_param;
    if((right_line_parm.is_empty() == 0)&&(left_line_parm.is_empty() == 0)){
        center_line_param = (left_line_parm + right_line_parm)/2;
    }else if(right_line_parm.is_empty() == 0){
        center_line_param = right_line_parm;
        center_line_param(2) = right_line_parm(2)-80;
    }else if(left_line_parm.is_empty() == 0){
        center_line_param = left_line_parm;
        center_line_param(2) = left_line_parm(2)+80;
    }

    if(center_line_param.is_empty() == 0){
        vec yc = linspace<vec>(0, 300, 300);
        vec xc = polyval(center_line_param,yc);

        for(int i=0;i<xc.n_elem;i++){
            //cout<<xr(i)<<" "<<yr(i)<<endl;
            if((xc(i)<300)&&(xc(i)>0)){
                temp.at<cv::Vec3b>(int(yc(i)),int(xc(i)))[0]=255;
                temp.at<cv::Vec3b>(int(yc(i)),int(xc(i)))[1]=255;
                temp.at<cv::Vec3b>(int(yc(i)),int(xc(i)))[2]=0;
            }

        }
        float center_offset = xc(xc.n_elem-1)-150;
        vec difff = diff(xc);
        float center_yaw = -(180/3.14)*tanh(difff(difff.n_elem-1));
        ret[0] = center_offset;
        ret[1] = center_yaw;
        ret[2] = center_line_param(0);
        //cout<<"center offset is:"<<center_offset<<"yaw offset is:"<<center_yaw<<endl;
